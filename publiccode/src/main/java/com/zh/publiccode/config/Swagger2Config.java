...
public class Swagger2Config {
    ...
    public Docket createRestApi() {
        return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo()).select()
                .apis(RequestHandlerSelectors.basePackage("com.zh.publiccode.controller")).paths(PathSelectors.any())
                .build();

    }

    private ApiInfo apiInfo() {
        return new ApiInfoBuilder().title("action-swagger").description("swagger").termsOfServiceUrl("")
                .version("1.0").build();
    }
/*    @Bean
    public Docket createRestApi() {
